...
Kaiteyo application builds accept `-PappDataSource=release` (default) or `-PappDataSource=source`. Both modes now produce the same small APK: the full SQLite database is not embedded in Compose resources. Source mode runs `:database:exportAppDatabase` in the CI data-preparation job, compresses the validated SQLite output as `kanji-dojo-data-base-v<version>.sql.gz`, and publishes the pack plus its SHA-256 checksum. At first launch, Android downloads the pack with resumable range requests, verifies the checksum, inflates it to the app database directory, validates the SQLite header, and atomically opens it. This preserves full offline search after hydration while keeping the APK below 70 MiB; the hydrated database itself still occupies its normal on-device storage footprint. If an existing installation cannot download the new pack, the provider retains a usable legacy database instead of deleting it before the replacement is verified. The `Build All` workflow exposes the same source/release choice and publishes the compressed pack when `publish_data_release` is enabled.
